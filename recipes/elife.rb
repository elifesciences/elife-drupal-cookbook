...
# # TODO: prevent this from running if we have already got these databases in place 
# # create a drupal db and eLife db 
# execute "add-drupal-db" do
#   command "/usr/bin/mysql -u root -p#{node[:mysql][:server_root_password]} -e \"" +
#       "CREATE DATABASE IF NOT EXISTS drupal;\""
#       "CREATE DATABASE IF NOT EXISTS elife;\""
#   action :run
#   ignore_failure true
# end

# # create elife user and grant priveldeges
# execute "create-elife-user" do
#   command "/usr/bin/mysql -u root -p#{node[:mysql][:server_root_password]} -e \"" +
#       "CREATE USER 'elife'@'localhost' IDENTIFIED BY '#{node[:mysql][:elife_user_password]}';"
#       "GRANT ALL PRIVILEGES ON elife.* TO 'elife'@'localhost' IDENTIFIED BY 'elife' WITH GRANT OPTION;" +
#   action :run
#   ignore_failure true
# end

# # load elife database dump 
# execute "load-elife-db" do
#   command "/usr/bin/mysql -u elife -p#{node[:mysql][:elife_user_password]} -e \"" +
#       elife < "drupal_prod_jnl_elife.latest.sql;\""
#   action :run
#   ignore_failure true
# end
...
bash "install-default-drupal-site" do
  ...
  drush make example.make www
  EOH
  not_if { File.exists?("/vagrant/public/drupal.vbox.local/www/index.php") }
end

# we are not using varnish for now, so don't set this
